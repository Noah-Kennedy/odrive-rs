...
fn test_set_current() {
    ...
    odrive.set_current(MotorAxis::Zero, 24.0).unwrap();
...
fn test_set_trajectory() {
    let mut odrive = init_odrive();
    odrive.set_trajectory(MotorAxis::Zero, 24.0).unwrap();
    assert_eq!(b"t 0 24\n".to_vec(), odrive.io_stream.get_mut().write_buffer);
    assert!(odrive.io_stream.get_mut().flushed)
}
...
fn test_set_velocity_default() {
    let mut odrive = init_odrive();
    odrive.set_velocity(MotorAxis::Zero, 24.0, None).unwrap();
...
fn test_set_velocity_feed_forward() {
    let mut odrive = init_odrive();
    odrive.set_velocity(MotorAxis::Zero, 24.0, Some(12.0)).unwrap();
...
fn test_set_position_p_default() {
    let mut odrive = init_odrive();
    odrive.set_position_p(MotorAxis::Zero, 24.0, None, None).unwrap();
...
fn test_set_position_q_default() {
    let mut odrive = init_odrive();
    odrive.set_position_q(MotorAxis::Zero, 24.0, None, None).unwrap();
...
fn test_get_velocity() {
    ...
    odrive.io_stream.get_mut().read_buffer.append(&mut b"25\n".to_vec());
    odrive.io_stream.get_mut().read_buffer.reverse();
    let result = odrive.get_velocity(MotorAxis::Zero).unwrap().unwrap();
    assert_eq!(25.0, result);
...
fn test_run_state_instant_switch() {
    ...
    odrive.io_stream.get_mut().read_buffer.reverse();
    let result = odrive.run_state(MotorAxis::Zero, AxisState::MotorCalibration, true).unwrap();
