...
fn test_set_pole_pairs() {
    ...
    odrive.set_motor_pole_pairs(MotorAxis::Zero, 25).unwrap();
...
fn test_set_resistance_calibration_max_voltage() {
    let mut odrive = init_odrive();
    odrive.set_motor_resistance_calib_max_voltage(MotorAxis::Zero, 25.0).unwrap();
...
fn test_set_requested_current_range() {
    let mut odrive = init_odrive();
    odrive.set_motor_requested_current_range(MotorAxis::Zero, 25.0).unwrap();
...
fn test_set_current_control_bandwidth() {
    let mut odrive = init_odrive();
    odrive.set_motor_current_control_bandwidth(MotorAxis::Zero, 25.0).unwrap();
